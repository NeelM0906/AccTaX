...
export default async function TabularReviewsPage({ searchParams }: PageProps) {
  const params = (await searchParams) ?? {};
  const workflowId = Array.isArray(params.workflow) ? params.workflow[0] : params.workflow;
  const workflows = getWorkflowCatalog("india").filter((workflow) => workflow.type === "tabular");
  const workflow = workflows.find((item) => item.id === workflowId) ?? workflows[0]!;
  const data = await getAssistantWorkspaceData("india");
  const records = [
    ...data.documents.map((document) => ({
      id: document.id,
      href: `/app/vault/${document.id}`,
    ...
    ...data.bills.map((bill) => ({
      ...
      amount: formatInr(Number(bill.total)),
      category: "expense",
    ...
    ...data.transactions.map((transaction) => ({
      ...
      amount: formatInr(Number(transaction.amount)),
  ...
  return (
    <div className="flex min-h-screen flex-col bg-white">
      <header className="flex shrink-0 items-center justify-between px-8 py-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button size="sm" asChild>
            <Link href={`/app/assistant?q=${encodeURIComponent(`Run ${workflow.title} on my records`)}`}>
      ...
      <div className="flex h-10 shrink-0 items-center justify-between border-y border-zinc-200 px-8">
        ...
        <Link href="/app/workflows" className="text-xs font-medium text-zinc-500 hover:text-zinc-900">Change workflow</Link>
      </div>
      ...
      <main className="grid flex-1 overflow-hidden xl:grid-cols-[minmax(0,1fr)_22rem]">
        ...
        <aside className="overflow-auto border-l border-zinc-200 p-5">
          ...
          <Button className="mt-5 w-full" asChild>
            <Link href={`/app/assistant?q=${encodeURIComponent(`Explain ${workflow.title}`)}`}>
