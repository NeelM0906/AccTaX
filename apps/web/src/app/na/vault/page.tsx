...
export default async function NorthAmericaVaultPage() {
  ...
  return (
    <div className="flex min-h-screen flex-col bg-white">
      ...
      <div className="flex h-10 shrink-0 items-center gap-6 border-y border-zinc-200 px-8 text-sm font-medium">
        ...
        <Link href="/na/invoices" className="text-zinc-500 hover:text-zinc-900">Invoices</Link>
      </div>
      ...
      <main className="grid flex-1 gap-8 px-8 py-6 xl:grid-cols-[minmax(0,1fr)_22rem]">
        <section className="min-w-0">
          ...
          {documents.map((document) => (
            <div key={document.id} className="grid h-11 grid-cols-[2rem_minmax(18rem,1fr)_10rem_10rem_10rem_2rem] items-center border-b border-zinc-100 text-sm hover:bg-zinc-50">
              ...
              <Link href={`/na/vault/${document.id}`} className="truncate px-2 font-medium text-zinc-900 hover:underline">{document.originalFilename}</Link>
          ...
          <div id="transactions" className="mt-10">
            ...
            {transactions.slice(0, 10).map((transaction) => (
              <div key={transaction.id} className="grid h-10 grid-cols-[8rem_minmax(18rem,1fr)_12rem_10rem] items-center border-b border-zinc-100 text-sm">
                <span className="text-zinc-600">{transaction.transactionDate.toLocaleDateString("en-US")}</span>
                ...
                <span className="text-right font-medium text-zinc-900">{formatCurrency(Number(transaction.amount), "USD")}</span>
              </div>
            ))}
          </div>
        </section>
        ...
        <aside className="space-y-4">
          <div className="rounded-2xl border border-zinc-200 p-4">
            ...
            <p className="mt-1 text-sm text-zinc-500">Receipts, bills, invoices, bank statements, and sales-tax source files.</p>
            <form action={uploadDocument} className="mt-4 space-y-3">
              <input type="hidden" name="returnTo" value="/na/vault" />
